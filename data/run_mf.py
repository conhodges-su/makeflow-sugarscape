...
def output_makeflow(config, configFiles, path):  
    with open(MAKEFLOW, 'w') as fout:
        output = ""
        for file in glob.glob("*.config"):
            # get the name of the file from the config_file list to output json name
            name = file.split(".")[0]
            # print the makeflow output and input files
            output += f"{name}.{JSON} : {file} {SUGARSCAPE} "
            # add the dependent modules to the dependencies
...
if __name__ == "__main__":
    ...
    output_makeflow(config, configFiles, path)
